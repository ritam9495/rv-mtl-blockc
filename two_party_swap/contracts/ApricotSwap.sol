...
contract TwoPartySwapApricot {
    ...
    /// stepTaken[0]=depositPremium; stepTaken[1]=Escrow; stepTaken[2]=RedeemAsset
    /**
    ...
    We do this by checking that both addresses are 0 by default. If they are not, the contract
    is currently in use (since 0 is not a valid address)
     */
   ...
   modifier canRedeemPremium(bytes32 hashLock) {
    ...
    function setup(uint expectedEscrowAlice,
        ...
        swaps[hashLock].sender = alice;//alice is sender
        swaps[hashLock].receiver = bob;//bob is receiver
        swaps[hashLock].stepsTaken = new bool[](3);

        //set up escrow mapping
        escrows[hashLock].expected = expectedEscrowAlice;
        escrows[hashLock].deadline = startTime + 3 * DELTA;
        escrows[hashLock].timeout = startTime + 6 * DELTA;
        
        //set up premium mapping
        premiums[hashLock].expected = expectedPremiumBob;
        premiums[hashLock].deadline = startTime + 2 * DELTA;
    ...
    /**
    In order for this function to go through, Bob must approve this contract to transfer on his behalf.
