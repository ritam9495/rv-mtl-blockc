...
const INDEX = 2;// INDEX shows the ith SIZE(512) protocols
...
contract(INDEX+1 + "th"+ SIZE + "Protocols", async (accounts) => {
  it("all", async function() {
    ...
    //have to allow Banana Swap to make transfers on alice & bob's behalf
    await che.increaseAllowance(carol_alice_edge.address, 10_000, { from: alice });
    await che.increaseAllowance(carol_alice_edge.address, 10_000, { from: bob });
    await che.increaseAllowance(carol_alice_edge.address, 10_000, { from: carol });
